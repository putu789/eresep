...
<?php
    ...
    <!-- #Top Bar -->
     <section class="content" style="margin-left: 13em;">
        <div class="container-fluid" >
  			<div class="col-md-10"> 
...
<?php }?>
                        ...
                        <div class="body">
                            ...
                            <td style="width:25%;">
                               ...
                               <td><select name="kode_obat" class="kd_obano form-control" style="width:100%"></select></td>
                                ...
                                <td style="margin-left:0px;"><input type="text" name="jumlah"   class="form-control" max="200" min="1"></td>
                                </tr>
                                </table>
                                <label>Aturan Pakai</label>
                                <br>
                                
                                <div class="col-md-2 "><input type="text" name="aturan[]" class="form-control"></div><div class="col-md-2" style="width:5px; padding-top:2px;"><input type="hidden" value="X" name="aturan[]">X</div> <div class="col-md-9"><input type="text" name="aturan[]" class="form-control"></div></div>
                                <br>
                                <table style="width:100%;">
                                <tr>
                ...
                $query_resep = query("SELECT a.kode_brng, a.jml, a.aturan_pakai, b.nama_brng, a.no_resep, b.ralan FROM resep_dokter a, databarang b, resep_obat c WHERE a.kode_brng = b.kode_brng AND a.no_resep = c.no_resep AND c.no_rawat = '".$no_rawat."' ");
				$no =1;
                while ($data_resep = fetch_array($query_resep)) {
					$obate = $data_resep['ralan'];
                    $enek_piro = $data_resep['jml'];
                    $tot_rego = $obate * $enek_piro;
                     for ($i=0; $i < count($enek_piro) ; $i++) { 
                      $total += $tot_rego;
                    }
					?>
                 <form method="post" action="">
                        ...
                        ?>
                          ...
                          <td><input type="number" name="jml" value="<?php echo $data_resep['1']; ?>" max="200" min="1"  ></td>
						   <td>Rp. <?php echo number_format($tot_rego,0,',','.');?></td>
			   ...
			   $no++;
					...
					<tr class="alert alert-info">
					  <td colspan="4"><b>Total Harga</b></td>
					   <td colspan="3"><b>Rp. <?php echo number_format($total,0,',','.') ;?></b></td>
					 </tr>
...
/*TINDAKAN */
	...
	function formatDataob (data) {
        var $data = $(
            '<b>'+ data.id +'</b> - <i>'+ data.text +'</i> - STOK:<b>('+ data.stok +')</b>- Harga:<b>(Rp. '+ data.regane +')</b>'
        );
        return $data;
    };
